# ReadDuck 讀鴨

在網頁與 PDF 上做雙語對照閱讀的瀏覽器擴充功能，支援 **Chrome 與 Microsoft Edge**。
翻譯、解釋、摘要**全部在你自己的電腦上跑** —— 用的是瀏覽器內建的裝置端 AI，
不需要 API key、不產生費用、看的內容不會離開這台機器。

[![Buy Me a Coffee](https://img.buymeacoffee.com/button-api/?text=Buy%20me%20a%20coffee&emoji=&slug=jonny030&button_colour=FFDD00&font_colour=000000&font_family=Cookie&outline_colour=000000&coffee_colour=ffffff)](https://buymeacoffee.com/jonny030)

互動設計參考沉浸式翻譯，引擎換成瀏覽器內建的 Translator / Language Detector / Prompt API：

| | Chrome | Microsoft Edge |
|---|---|---|
| Translator / Language Detector | [138 起](https://developer.chrome.com/docs/ai/translator-api) | [148 起](https://learn.microsoft.com/microsoft-edge/web-platform/translator-api) |
| Prompt API（摘要／解釋／問答） | [138 起](https://developer.chrome.com/docs/ai/prompt-api)，Gemini Nano | [138.0.3309.2 起](https://learn.microsoft.com/microsoft-edge/web-platform/prompt-api)，Phi‑4‑mini，**限 Canary / Dev 且需自行開啟 flag** |

同一份程式碼、同一個 zip 兩邊都能裝；差異（版本門檻、內部頁面網址、可用的輸出語言）
由 `src/lib/browser.js` 在執行時判斷。

---

## 使用方法

1. 開啟 `chrome://extensions`（Edge 是 `edge://extensions`），打開「開發人員模式」
2. 點「載入未封裝項目」，選這個資料夾（零建置 —— 不必 `npm install`，不必打包）
3. 隨便開一個英文網頁，點右下角的鴨子 🦆

**Edge 使用者請先看〈[在 Microsoft Edge 上](#在-microsoft-edge-上)〉** ——
摘要／解釋／問答目前需要 Canary 或 Dev 版並手動開一個 flag。

第一次翻譯會下載該語言對的翻譯模型，很小，通常幾秒就好。
**摘要、解釋、簡化、問答用的是另一套數 GB 的語言模型**，第一次用到時會先問過你才開始下載 ——
詳見〈[兩套模型](#兩套模型)〉。

---

## 功能與用法

### 1. 雙語對照翻譯網頁

譯文插在原段落下方，原文保留，兩行對著看。只翻譯視窗附近的段落，捲到哪翻到哪；
無限捲動的網站新載入進來的內容也會自動接上。

**三個入口，做同一件事：**

- 點頁面右下角的鴨子
- 在頁面上按右鍵 →「切換雙語對照翻譯」
- 點工具列的 ReadDuck 圖示 → 最上面那個開關

翻譯進行中，鴨子右下角會顯示還剩幾段沒翻完。再點一次就整頁還原，不留痕跡。

**隨手可調的三件事**（工具列圖示的小視窗裡）

| 設定 | 選項 |
|---|---|
| 翻譯成 | 39 種語言，預設繁體中文 |
| 譯文樣式 | 虛線底線（預設）／淡色底／模糊（滑過才顯示）／無樣式 |
| 在這個網站自動翻譯 | 打開後，這個網域一開頁就自動翻，不必再點 |

頁面本來就是目標語言時不會重翻一次，會直接告訴你「這個頁面已經是繁體中文」。
中英夾雜的頁面則逐段判斷，只翻外文的那幾段。

### 2. 劃選即譯 / 解釋 / 簡化

用滑鼠選取任何一段文字，旁邊會浮出一條小工具列：

- **譯** —— 翻成你的目標語言
- **解釋** —— 說明裡頭的術語與言外之意，逐字串流輸出
- **簡化** —— 改寫成更好懂的說法
- **摘要側欄** —— 把這段帶進側邊欄繼續追問

按 `Esc` 可以中止進行中的生成，結果面板上有「複製」。
在輸入框裡選字**不會**跳出工具列（免得打字時被打擾）。
不想要這條工具列的話，設定頁可以關掉 —— 右鍵選單裡一樣有「翻譯／解釋／簡化選取的文字」。

### 3. 整頁摘要與問答（側邊欄）

側邊欄會先抽出這一頁的正文（自動去掉導覽列、頁尾、側欄那些雜訊），然後：

- **產生摘要** —— 一句話結論 + 重點清單 + 名詞解釋
- **接著提問** —— 底下的輸入框直接問，模型記得整篇內容與前面幾輪對話
- 底部顯示上下文用量，滿了或想換話題就按「重設對話」

**怎麼開啟：** 滑鼠移到右下角的鴨子上 → 點展開的「摘」；或右鍵 →「開啟摘要側邊欄」；
或工具列圖示 →「摘要側欄」。

超過上下文長度的長文會自動分段閱讀再合併，不會直接失敗。
**PDF 不支援摘要與問答** —— PDF 只做雙語對照翻譯。

### 4. PDF 翻譯

瀏覽器內建的 PDF 檢視器沒辦法加譯文，所以 ReadDuck 附了自己的檢視器。

**怎麼開啟：**

- 用瀏覽器開著 PDF 時，點右下角的鴨子 —— 會用 ReadDuck 的檢視器重開同一份檔案
- 網頁本身只是包著一份 PDF 時（例如 IEEE Xplore 的全文頁），鴨子也會直接開啟裡面那份 PDF
- 在 PDF 網址上點工具列圖示 →「翻譯 PDF」（這時它會是主要按鈕）
- 在指向 PDF 的連結上按右鍵 →「用 ReadDuck 翻譯這個 PDF」
- 本機檔案：打開檢視器後按「開啟本機檔案」，或直接把檔案拖進去（不需要開啟「允許存取檔案網址」）

**兩種呈現方式**，工具列的按鈕隨時切換，切換不必重新翻譯：

- **左右對照**（預設）—— 左邊乾淨的原文，右邊同一頁疊上譯文。版面完全一致，可以直接對照
- **只看譯文** —— 只留疊上譯文的那一份，適合專心讀

同一頁一欄、兩欄混著出現也認得出來（全寬摘要底下接雙欄正文、橫跨兩欄的圖說），
頁邊放出處與作者單位的窄側欄也會和正文分開翻。
有些期刊把表格的字畫成圖形（PDF 裡沒有文字可抽），ReadDuck 會自動找出來、用裝置端語言模型
辨識後再翻譯，譯文一樣疊回原位；第一次需要時會先詢問是否下載模型。
表格會一格一格翻，照列的順序讀，不會被當成兩欄從中間切開；論文側邊的直立浮水印與
圖表的縱軸標籤會略過，只有數字的格子與公式不送去翻，原文的公式保持原樣。

譯文直接蓋在原文位置上，維持論文原本的版面（含雙欄）：字色、粗細、襯線或無襯線、
行距、對齊與縮排都照原文，條列項目的懸掛縮排也保留。滑鼠滑過**不會**淡出；
想核對原句的話，停在譯文上會顯示原文提示。
頁面是捲到才處理的，幾百頁的文件不會一開啟就卡住。

### 5. 圖片文字翻譯

在任何圖片上按右鍵 → **「翻譯圖片中的文字」**。模型先把圖裡的文字逐字讀出來，
再交給翻譯模型轉成你的目標語言 —— 面板上譯文在前、原文在後，方便核對。

**只有 Chrome 支援**（Edge 的 Phi-4-mini 是純文字模型）。

譯文顯示在浮動面板裡，**不會疊回圖片上** —— 模型只給文字不給座標。

模型實際只看得到 768×768 的畫面，整張大圖丟進去的話小字會糊掉，所以圖片會先切塊
再逐塊辨識，面板上會顯示「辨識中… 3/9 塊」。切得愈多次數愈久，這也是為什麼它是
右鍵觸發、不會自動處理整頁的圖片。

如果你在 `chrome://flags` 開了「Experimental Web Platform features」，ReadDuck 會改用
系統 OCR 先找出文字在哪，只裁有文字的區塊 —— 辨識更清楚、呼叫次數也更少。沒開也能用，
只是走等距切塊。

圖片被網站保護（防盜連、需要登入）而讀不到時，ReadDuck 會自動改用**畫面擷取** ——
擷取的是你螢幕上已經顯示的內容，所以一定拿得到。這時只涵蓋看得見的部分，把圖片捲到
完整可見可以讀得更全，面板上也會標註。

真正讀不到的情況：掃描品質太差、藝術字、或圖片根本沒顯示在畫面上。

### 6. 輸入框翻譯

在任何輸入框（`textarea`、留言框、Gmail 編輯區）打好中文，按右鍵 →
**「翻譯這個輸入框的內容」**，會就地改寫成外文。這裡用的是語言模型而不是翻譯模型，
因為要的是符合語境的自然表達而不是直譯。改壞了可以「復原」還原成原本打的字。
輸出語言在設定頁單獨設定（預設英文），跟閱讀用的目標語言分開。

---

## 入口一覽

同一件事通常有好幾條路，挑順手的就好：

| 想做的事 | 浮動鴨子 | 右鍵選單 | 工具列圖示 |
|---|---|---|---|
| 切換雙語對照 | 點一下 | ✓ | ✓（開關） |
| 開設定頁 | 滑上去 → ⚙ | | ✓（右上 ⚙） |
| 開摘要側邊欄（PDF 不支援） | 滑上去 →「摘」 | ✓ | ✓ |
| 翻譯／解釋／簡化選取的文字 | 選字後的浮動工具列 | ✓ | |
| 翻譯輸入框內容 | | ✓（在輸入框上按） | |
| 翻譯圖片中的文字 | | ✓（在圖片上按） | |
| 用 ReadDuck 開 PDF | 點一下（在 PDF 頁面上） | ✓（連結上或 PDF 頁面上） | ✓ |
| 內建 AI 診斷 | | | ✓ |

浮動鴨子可以在設定頁關掉，關掉之後其他入口都不受影響。

---

## 兩套模型

ReadDuck 會用到兩套彼此獨立的模型 —— 這是瀏覽器的架構決定的，不是本專案的選擇。
官方文件的說法是「Translator 和 Language Detector API 使用專家模型，其餘所有 API 使用語言模型」：

| 功能 | 模型 | 大小 | 何時下載 |
|---|---|---|---|
| 雙語對照、劃選即譯、PDF 翻譯 | 翻譯專家模型（依語言對） | 小 | 第一次翻該語言對時 |
| 摘要、解釋、簡化、問答、輸入框改寫 | 基礎語言模型 | **數 GB** | 第一次用到這些功能時 |

所以**翻譯已經能用，不代表摘要也能馬上用**。側邊欄一開啟就會檢查語言模型在不在，
並把狀態顯示出來：已就緒、尚未下載（附「下載模型」按鈕），或這台裝置不支援（列出缺什麼）。
那是一筆很大的下載，任何情況下都會先問過你才開始；下載完成後所有 AI 功能就都能直接使用。

瀏覽器規定模型下載必須由使用者操作觸發，所以第一次一定要你自己點一下按鈕；
之後設定成自動翻譯的網站就能一開頁直接翻。

想看目前的模型狀態：工具列圖示 →「診斷」，或直接開 `chrome://on-device-internals`
（Edge 是 `edge://on-device-internals`）。

---

## 設定

工具列圖示 → 右上角 ⚙，或右下角鴨子滑上去點 ⚙。

- **外觀** —— 深淺色：跟隨系統（預設）／淺色／深色。套用在設定頁、側邊欄、
  工具列小視窗、診斷頁、PDF 檢視器，以及頁面上的浮動 UI
- **語言** —— 閱讀的目標語言、輸入框翻譯的目標語言（兩者分開）
- **譯文外觀** —— 樣式、字級（相對原文 70–130%），改的當下就有預覽
- **行為** —— 是否顯示浮動鴨子、是否顯示劃選工具列、同時翻譯的段落數、最短翻譯長度
- **網域規則** —— 自動翻譯清單、永不翻譯清單（一行一個網域）
- **譯文快取** —— 開關、目前用量、清除。翻過的段落會存下來，重訪同一頁幾乎瞬間出現
- **提示詞** —— 七則 system prompt 全部可以改（解釋／簡化／摘要／分段摘要／問答／圖片辨識／輸入框翻譯），
  改了立刻生效，每則都能單獨回復內建
- **其他** —— 內建 AI 診斷、回復預設值

### 關於改提示詞

**請用英文寫。** 模型保證的輸出語言不含中文，ReadDuck 走的是「模型用英文輸出 →
翻譯模型轉成你的目標語言」；用中文寫指示反而容易讓模型混淆。

輸入框裡就是目前實際在用的提示詞，直接改它。「用哪種語言回答」那一行由程式自動接在最後、
**不開放編輯** —— 整條雙段管線靠它，刪掉會直接壞掉；欄位底下會顯示那一行實際長什麼樣，
不用自己再寫一次。要在文字裡提到目標語言請用 `{language}`。

改回和內建一模一樣就等於沒有自訂（不會佔用同步儲存空間），也可以按「回復內建」。

---

## 在 Microsoft Edge 上

Edge 也是 Chromium，API 形狀與 Chrome 一模一樣，所以功能完全相同。要注意三件事：

**1. 翻譯需要 Edge 148 以上。** 雙語對照、劃選即譯、PDF 翻譯只要版本夠就能直接用。

**2. 摘要／解釋／簡化／問答還在開發者預覽。** 需要 Edge **Canary 或 Dev**
（138.0.3309.2 以上），並且手動開啟 flag：

1. 開 `edge://flags`，搜尋 **Prompt API for on-device language model**
2. 設為 **Enabled**，重新啟動 Edge
3. 開 `edge://on-device-internals`，確認「裝置效能類別」是 **High** 以上

效能類別是 Medium 或 Low 的話 Phi‑4‑mini 不會啟用，可以改用較小的 Aion‑1.0‑Instruct ——
需要 Edge 150.0.4070 以上，另外開啟 **Enable pre-release on-device language model** flag。

**3. Edge 上的摘要一律先用英文生成，再由 Translator API 轉成你的目標語言。**
Chrome 的 Prompt API 保證 `en / ja / es / de / fr` 五種輸出語言，Microsoft 沒有對
Phi‑4‑mini 做同樣的承諾，所以 ReadDuck 不賭它的多語能力 —— 讓專用翻譯模型做翻譯，
結果比讓小模型直接寫外語穩定。中文使用者在 Chrome 上本來就走這條路，體感沒有差別；
日／西／德／法使用者在 Edge 上會多一次轉譯。

診斷頁（工具列圖示 →「診斷」）會直接顯示目前判定成哪個瀏覽器、flag 有沒有開、
以及模型輸出語言會怎麼走。

---

## 系統需求

內建 AI 對硬體有實際門檻，不符合的話擴充功能會直接告訴你缺什麼。

**Chrome**

- **Chrome 138 以上**（桌機版；行動版不支援）
- Windows 10/11、macOS 13+、Linux 或 ChromeOS 16389.0.0+
- **至少 22 GB 可用硬碟空間**（模型檔案）
- 獨立 GPU（4 GB 以上 VRAM）**或** 16 GB RAM 加 4 核心以上 CPU
- 首次下載模型時需要非計量網路連線

**Microsoft Edge**

- 翻譯：**Edge 148 以上**；摘要／解釋／問答：**Canary 或 Dev 138.0.3309.2 以上 + flag**
- Windows 10/11 或 macOS 13.3 以上
- **至少 20 GB 可用硬碟空間**（低於 10 GB 時模型會被自動刪除）
- **5.5 GB 以上 VRAM**，且「裝置效能類別」為 High 以上
- 首次下載模型時需要非計量網路連線（計量連線不會下載模型）

不確定這台機器行不行？安裝後開**診斷頁**（工具列圖示 →「診斷」），它會實測並列出結果。

---

## 遇到問題

| 症狀 | 原因與處理 |
|---|---|
| 頁面上沒有鴨子 | `chrome://` / `edge://`、擴充功能商店這類瀏覽器內部頁面不支援；或設定頁把它關掉了；剛安裝／剛重新載入擴充功能的話，已開著的分頁要重新整理一次 |
| 摘要說「讀不到這個頁面的內容」 | 同上，重新整理那個分頁即可 |
| 按了翻譯卻沒動靜 | 第一次會下載翻譯模型，開診斷頁看模型狀態 |
| 摘要／解釋要求下載數 GB | 那是語言模型，跟翻譯是兩套。看〈[兩套模型](#兩套模型)〉 |
| 說這台裝置不支援 | 開診斷頁，它會列出實際缺的是哪一項 |
| Edge 上翻譯可以、摘要卻說沒有 Prompt API | Prompt API 還在開發者預覽，見〈[在 Microsoft Edge 上](#在-microsoft-edge-上)〉 |
| PDF 開起來說「沒有可抽取的文字」 | 掃描檔（純圖片）沒有文字層，抽不出東西；ReadDuck 不做 OCR |
| 線上 PDF 開起來說「伺服器回傳的是網頁」 | 多半是要登入的期刊 PDF，或下載連結已經過期（ScienceDirect 那類有簽章的連結只有幾分鐘效期）。先在瀏覽器裡把 PDF 打開，再點 PDF 上的鴨子 —— ReadDuck 會沿用那個分頁已經載入的檔案 |
| PDF 上按摘要說不支援 | PDF 只做翻譯，不做摘要與問答 |
| 某個網站翻譯失敗 | 該網站可能用 `Permissions-Policy` 擋掉翻譯 API，ReadDuck 會自動改走備援路徑，此時沒有串流效果 |

---

## 授權

MIT（見 `LICENSE`）。內建的 PDF.js 為 Apache-2.0，見 `vendor/pdfjs/`。
